...
export async function executeBookingSaga(unsafePayload) {
    ...
    try {
        ...
        if (!email) {
            throw createBookingError(ERROR_CODES.INVALIDPAYLOAD, "Email is required", { traceId: traceId });
        }
        ...
        if (!serviceId || !_looksLikeGuid(serviceId)) {
            throw createBookingError(ERROR_CODES.SERVICENOT_FOUND, "Service not found", {
                traceId: traceId,
        ...
        if (!f1LocalStart || !f1LocalEnd) {
            throw createBookingError(ERROR_CODES.INVALIDPAYLOAD, "F1 slot dates are required", {
                traceId: traceId,
            });
        }
        ...
        if (existingCitaRes?.items?.length > 0) {
            ...
            if (existingPaymentStatus === ESTADO_PAGO.PENDINGPAYMENT) {
        ...
        if (!txResult.success) {
            ...
            if (txResult.error === "TRANSACTIONPREVIOUSLYFAILED") {
                throw createBookingError(
                    ERROR_CODES.BOOKINGCREATION_FAILED,
        ...
        if (resourceValidation?.status !== "SUCCESS") {
            ...
            throw createBookingError(
                resourceValidation?.error?.code || ERROR_CODES.SLOTUNAVAILABLE,
        ...
        saga.addStep(
            ...
            async function () {
                    for (const lockKey of lockKeys) {
                        const lockResult = await _lockSlotKeyOrFail(lockKey, lockOwnerId, LOCKTTL_MS);
                        if (!lockResult?.ok) {
                            throw createBookingError(
                                ERROR_CODES.TOKENBUSY,
                    ...
                    heartbeatInterval = setInterval(function () {
                        lockKeys.forEach(function (key) {
                            _renewLock(key, lockOwnerId, LOCKTTL_MS).catch(function () {});
                        });
                    }, HEARTBEAT_MS);
        ...
        const paymentStatus = isOnline ?
            ESTADO_PAGO.PENDINGPAYMENT :
            ESTADO_PAGO.UNPAID;
        const citaStatus = isOnline ?
            ESTADO_CITA.PENDINGPAYMENT :
        ...
        return {
            ...
            data: null,
            error: {
                code: norm.code || ERROR_CODES.UNKNOWNERROR,
